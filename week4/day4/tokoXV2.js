...
const countProfit = shoppers => {
  ...
  if (shoppers.length) {
    for (let i = 0; i < listBarang.length; i++) {
      ...
      result[listBarang[i][0]].leftOver = listBarang[i][2]
    }
    for (let j = 0; j < shoppers.length; j++) {
      let check = false
      console.log(result)
      console.log(result[shoppers[j].product], j)
      if (result[shopper[j].product].leftOver >= shoppers[j].amount) {
        if(result[shoppers[j].product].shoppers == shoppers[j].name) {
          check = true
        }
        if(!check) {
          result[shoppers[i].product].leftOver -= shoppers[j].amount
          result[shoppers[i].product].shoppers.push(shoppers[j].name)
        }
      }
    }
  }

...
list uang ada 5000, 10000, 20000, 50000, 100000
*/
function hitungKembalian(bayar, harga){
  ...
  for (let i = 0; i < ketersediaanUang.length; i++) {
    let temp = 0
    for (key in ketersediaanUang[i]) {
      while (kembalian >= ketersediaanUang[i].nominal && temp < ketersediaanUang[i].jumlah) {
        kembalian -= ketersediaanUang[i].nominal
        if (kembalian - ketersediaanUang[i].nominal) {
          temp++
        }
        obj[ketersediaanUang[i].nominal] = ketersediaanUang[i].jumlah - (ketersediaanUang[i].jumlah - temp)
      }
    }
  }
  return obj
